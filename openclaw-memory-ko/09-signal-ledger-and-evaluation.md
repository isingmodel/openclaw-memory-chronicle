...
기억 시스템의 역사는 단순한 기능 목록보다 “어떤 실패가 무엇을 드러냈고, 프로젝트가 어떻게 반응했는가”로 읽을 때 더 선명해진다. 다음 장부는 기준점인 2026-07-18까지 확인된 신호, 당시의 대응, 아직 답하지 못한 질문을 한 줄로 연결한다. 왼쪽에서 오른쪽으로 읽으면 개별 사건이 설계 교훈을 거쳐 아직 검증하지 못한 주장으로 바뀌는 과정을 볼 수 있다.
...
| [이슈 #16670](https://github.com/openclaw/openclaw/issues/16670): 설정 후 메모리가 고장처럼 보임                                     | 제공자 온보딩과 상태 진단도 체감 메모리 품질의 일부다                   | FTS 폴백, 제공자 `none`, 진단·상태 기능 강화                                                                | “파일은 있음 / 인덱스가 오래됨 / 제공자 사용 불가 / 허용된 결과 없음”을 하나의 추적으로 설명할 수 있는가?             |
...
| [이슈 #45608](https://github.com/openclaw/openclaw/issues/45608): 초기화 전 에이전트식 플러시 부재                                   | 기계적 대화 기록 포획과 의미 통합은 같지 않다                           | 기준점에서도 열려 있음                                                                                      | `/new`에 제한된 큐레이션을 넣어야 하는가? 지연과 데이터 손실의 균형은 어떻게 잡는가?                                  |
...
| [이슈 #99611](https://github.com/openclaw/openclaw/issues/99611): 표면을 넘나들 때의 기억상실                                        | 개인 비서 식별 정보에 비해 세션 키가 지나치게 좁았다                    | 보호된 `rememberAcrossConversations`. 기준점의 `main`에만 있고 출시 태그에는 없음                           | 열거한 모든 바인딩·DM 배치 적합성 사례에서 관찰된 교차 사용자 회상이 0이고, 식별 불가 대상은 계속 실패 시 차단되는가? |
...
평가용 말부터 일상어로 맞추자. **합성 평가 실행**은 실제 사적 대화 대신 정답과 허용 범위를 미리 정한 가짜 대화로 시스템을 시험하는 실행이다. **운영 환경 수집**은 실제 사용 중의 사건을 기록하는 일이므로 별도의 명시적 동의가 필요하다. **의미 산출물**은 사람이 나중에도 뜻을 알아볼 수 있는 메모리 기록이고, **생명주기 소유자**는 그 기록·색인·삭제 단계를 책임지는 런타임이나 플러그인이다. **평가 사례(fixture)**는 입력과 기대 결과를 묶은 시험 한 건이며, 그 기대 결과를 **정답 표지(label)**라고 부른다.
...
이 추적이 있으면 “에이전트가 잊었다”를 하나의 모호한 실패로 취급하지 않아도 된다. 원인은 쓰기 부재, 오래된 인덱스, 범위(scope) 거부, 검색 누락, 순위 경쟁에서의 탈락, 문맥(context) 잘림, 관찰 가능한 참조 부재, 또는 낡은 내용일 수 있다. 각각의 수정 방법은 다르다.
...
| 제공자 장애                   | FTS나 실패 시 닫힘 대신 검색이 조용히 빈 결과를 반환            |
...
하나의 “메모리 성공률”은 CJK, 제공자, 범위 수정의 계기가 된 구체적 회귀를 숨긴다.
...
두 실험군은 동일하게 권한이 확인된 코퍼스와 범위를 받아야 한다. 기본은 합성 자료다. 비공개 운영 메모리를 쓰려면 명시적 동의가 필요하며, 두 번째 판정 모델 제공자에게 보내는 경우 그 동의도 포함해야 한다. 도구는 기록 전용으로 두고 외부 메시지·행동은 억제하거나 중복 제거하며, 두 실행의 상태를 격리한다.
