...
> OpenClaw의 내장 영속 의미 메모리는 사용자가 검사할 수 있는 Markdown이다. 검색 인덱스와 운영·통합 상태는 파생되거나 기계가 소유하며, 공개되지 않은 또 하나의 정본이 되어서는 안 된다.
...
2. **정본 처리:** Markdown, 대화 기록, LanceDB처럼 정본을 소유한 쪽이 삭제 또는 교정을 수행한다. 파생 인덱스만 지워 원본에서 다시 살아나는 상태를 성공으로 보지 않는다.
