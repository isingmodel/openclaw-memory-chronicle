...
2026-07-18 기준 `main`에서 가장 즉각적인 세 간극은 문서와 런타임의 비공개 부트스트랩 불일치, Dreaming 저장소 문서의 노후화, 대화 기록 구체화의 별도 범위 경계다. 이 장은 여기에 의미 설명, 삭제, 백업·복구·내보내기, 재설정 포획, 플러그인 적합성과 도구 라우팅의 계약 공백도 함께 기록한다. 마지막에는 역사에서 도출한 별도 관리 제안으로 능력 스캐폴딩의 수명주기를 다룬다. 앞의 현재 간극 판단은 오래된 이슈 설명을 옮긴 것이 아니라 고정본의 소스, 테스트, 문서를 대조한 결과다.
...
## 별도 관리 제안 · 제안 우선순위 P1: 가드 등록부를 만든다

2장부터 7장까지의 역사는 기능과 가드가 언제, 어떤 압력 아래에서 추가되었는지는 추적한다. 그러나 이 책이 확인한 공개 소스·문서에서는 과거 모델의 도구 미호출이나 첫 답변 회상 실패를 보완한 장치를 어떤 조건에서 다시 평가하고 완화·교체·제거할지 정한 일관된 관리 계약을 찾지 못했다. 이는 특정 코드 한 줄의 현재 결함이나 유지보수자가 선언한 계약 간극이 아니라, 역사에서 도출한 이 책의 관리 제안이다.

모든 가드에 만료일을 붙여 자동 삭제하자는 뜻은 아니다. 권한, 데이터 무결성, 삭제, 승인, 원자성과 감사 가능성은 모델 성능이 높아져도 유지할 결과 계약이다. 수명주기 검토가 필요한 대상은 회상 시점 강제, 고정 도구 순서, 선택 플러그인이 활성화된 경로의 응답 전 별도 회상, 프롬프트 예외와 복구 휴리스틱처럼 특정 능력 부족을 보완하는 후보 스캐폴딩이다. 청킹·상위 K·재순위화 같은 검색 기본값은 인덱스 표현과 자원 예산도 함께 책임질 수 있으므로, 능력 보완을 목적으로 둔 경우에 한해 같은 등록부에서 그 역할을 분리해 적는다.

각 항목에는 최소한 다음 정보를 연결해야 한다.

- 안정된 가드 식별자, 구현·설정 위치, 기본 활성화 여부와 변경 이력
- 추가를 촉발한 사건·가설과 직접 근거
- 재현된 모델·프롬프트·백엔드·작업 분포
- 보호 대상이 안전 불변식, 제품 의미 계약, 능력 보완 가운데 무엇인지
- 연결된 회귀 평가 사례와 기대한 지표 개선
- 허용하지 않을 부작용, 담당 소유자와 적용 범위
- 모델·프롬프트·도구 스키마·백엔드·검색 정책·작업 분포 등 적용 조건이 실질적으로 달라질 때의 재검토·완화 조건
- 축소·비활성화·제거를 승인할 평가와 단계적 복구·되돌림 경로

등록부는 모든 가드를 가시화하되, 제거 평가는 보호 대상을 분류한 결과가 능력 스캐폴딩인 항목에만 적용한다. 안전·무결성·가용성 불변식은 제거 후보가 아니며, 제품 의미 계약을 바꾸려면 별도의 정책 결정과 버전 관리가 필요하다.

이 등록부는 과거 회귀 시험을 지우지 않는다. “옛 실패가 다시 생기지 않는가”와 함께 “이 가드가 현재 조합에서도 순효용을 주는가”를 묻게 한다. 모델별 하네스 분기가 필요하더라도 권한·정본·삭제 계약이 갈라져서는 안 된다. [11장](11-benchmarks-and-roadmap.md)은 이 관리 정보를 스캐폴딩 제거 평가와 모델×작업×하네스 행렬로 바꾸는 방법을 제안한다.
...
현재 계약의 간극은 그대로 평가 의무가 된다. 비공개 부트스트랩, 대화 기록 구체화, 저장소 설명, 오염 방지, 삭제, 백업·복구, 플러그인 소유권이 합의되지 않으면 높은 회상 점수도 범위와 수명주기를 지켰다는 증명이 될 수 없다. 별도로 과거 가드의 목적과 재검토 조건이 없으면 회귀를 막는 장치가 현재 모델·작업 조합에서도 순효용을 주는지 알 수 없다.
