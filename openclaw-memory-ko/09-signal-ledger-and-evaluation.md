...
앞 장의 설계 원칙은 무엇을 지켜야 하는지 말해 주었다. 그러나 프롬프트보다 런타임 강제를, 사이드카보다 SQLite를, 무제한 회상보다 범위 경계를 택했다는 사실만으로 그 선택이 실제 작업을 개선했는지는 알 수 없다. 이제 역사를 변화의 목록이 아니라 검증할 가설의 목록으로 다시 읽어야 한다.

OpenClaw에는 이미 버그 보고, QA 시나리오, 검색 진단, 인용, 회상 횟수, Dreaming 검토처럼 풍부한 신호가 있다. 부족한 것은 이 신호를 “정보가 제시됨”에서 “나중 작업의 결과”까지 이어 주는 분기형 결과 추적 체계다. 저장, 인덱스, 범위 판정, 검색, 주입, 참조 가운데 어디에서 결과가 갈렸는지를 구분해야 각각에 맞는 수정을 고를 수 있다.

이 장은 먼저 2026-07-18까지 확인된 신호를 현재의 증거로 정리한 뒤, 개별 사건이 어떤 주장까지 뒷받침하는지 한계를 긋고 그 증거에서 평가 설계를 끌어낸다. 뒤에 나오는 추적 단계, 지표, SLO, 실험 순서는 모두 이 책이 검토하는 제안이며, OpenClaw가 이미 채택했거나 보장하는 제품 계약이 아니다.
...
| [이슈 #20730](https://github.com/openclaw/openclaw/issues/20730): CJK에서 BM25 실패                                                  | 영어 중심 집계 테스트가 언어권 전체의 회상 실패를 가렸다                | trigram 토크나이저, 짧은 질의 폴백, 재인덱싱 식별 정보                                                      | 현실적인 검색 구간이 아직 부족한 문자 체계와 언어는 무엇인가?                                                         |
...
| [이슈 #72015](https://github.com/openclaw/openclaw/issues/72015): Active Memory/[QMD](01-current-architecture.md#qmd-explained) 지연 | 선제 회상은 응답성을 낮추고 초기화 부하를 키울 수 있다                  | 기한, 자격 판정, 실패 시 계속 동작을 추가했지만 이슈는 열려 있음                                            | 1밀리초 또는 한 번의 모델 호출당 얻는 효용은 얼마인가?                                                                |
...
| [이슈 #100978](https://github.com/openclaw/openclaw/issues/100978): 대화 기록 발견 수단 부재                                         | 의미 메모리와 정확한 아카이브 조회는 서로 다른 일이다                   | FTS5 기반 `sessions_search`                                                                                 | 에이전트는 언제 대화 기록 검색을 쓰고 언제 장기 의미 메모리 검색을 써야 하는가?                                       |
...
OpenClaw가 가진 신호는 이미 충분히 다양하다. 문제는 이 신호가 **정보가 제시된 순간**부터 **미래 작업 결과**까지 이어지는 관찰 가능한 사건 그래프로 결합되지 않았다는 데 있다.

이 그래프는 하나의 직선 깔때기여서는 안 된다. 가져오기는 `Captured` 단계에서 시작할 수 있고, 시작 시점 주입은 검색 질의를 건너뛸 수 있으며, 정확한 대화 기록 검색은 의미 인덱스를 우회한다. 미리 적재한 검색 벤치마크는 수명주기 포획 자체를 시험하지 않는다. 따라서 상관관계 ID는 실제로 존재하는 사건만 연결해야 하며, 모든 경로가 같은 단계를 밟는다고 암시해서는 안 된다. 그래프는 실패 위치를 찾게 해 주지만, 인과적 의존은 반복된 기준선·후보 대조 실험으로 따로 추정해야 한다.
...
## 한 사건은 보장을 반증하지만 후보 정책을 승인하지 않는다

비결정적인 LLM 시스템에서도 개별 사건은 강한 증거가 될 수 있다. 다만 그 사건으로 어떤 주장을 하려는지 먼저 구분해야 한다. 파일 내용이 한 번 사라졌다면 “현재 경로에서는 내용 손실이 불가능하다”는 보편적 보장은 이미 무너진다. 권한 없는 기억이 한 번 주입되거나 회상 보조 경로가 한 번 시간 상한을 넘겼다면 프라이버시·가용성 계약도 각각 마찬가지다.

그러나 그 사건을 겨냥해 만든 새 프롬프트나 검색 정책이 전체적으로 더 낫다는 주장은 별개다. 한 실행은 실패의 **도달 가능성**을 보여 줄 수 있지만 실패의 **빈도**를 추정하지 못하고, 후보 정책의 **평균 효용**이나 다른 작업에서의 부작용을 입증하지도 못한다.

| 판단하려는 주장                                  | 사건 한 건의 증거력 | 바로 정당화되는 대응                | 추가로 필요한 증거                                 |
| ------------------------------------------------ | ------------------- | ----------------------------------- | -------------------------------------------------- |
| 해당 실패가 도달 가능한가                        | 강함                | 최소 재현 사례와 회귀 시험으로 보존 | 코드·상태 추적으로 실패 위치 확인                  |
| 데이터 손실·권한 누출·시간 상한 위반이 가능한가 | 매우 강함           | 안전한 차단과 불변식 강화           | 차단이 다른 계약을 깨지 않는지 검증                |
| 실패가 얼마나 자주 일어나는가                    | 약함                | 계측 시작                           | 자격 있는 전체 실행 수와 원인별 발생률             |
| 후보 프롬프트·랭커가 전체적으로 더 나은가        | 매우 약함           | 후보 가설로 등록                    | 반복 대응 표본 시험 또는 무작위 대조 시험, 장기 궤적, 비용·안전 비교 |
| 사건이 후속 변경의 직접 원인이었는가             | 사건만으로는 불충분 | 맥락적 신호로 기록                  | 연결된 이슈·PR·커밋 설명                           |

> **폴백은 새 정책에 투표하는 한 표가 아니라, 1차 경로가 실패하거나 저하된 위치를 알려 주는 센서다.**

따라서 개별 사건은 현재 정책의 해당 보장을 반증하거나 해당 경로를 기각할 수 있어도 후보 정책을 승인하지는 못한다. 이 **증거 비대칭**을 놓치면 실제 결함을 발견한 정당한 긴급성이 곧바로 광범위한 기본값 변경의 정당성으로 바뀐다.

### 비결정성보다 더 어려운 것은 정책 개입의 비국소성이다

프롬프트 변경의 위험을 출력의 무작위성만으로 설명하면 부족하다. 온도를 낮춰 같은 입력의 출력을 안정시켜도, 여러 작업이 공유하는 자연어 정책 한 줄을 바꾸면 모델의 주의 배분, 도구 선택, 검색 시점, 계획 길이와 컨텍스트 소비가 함께 달라질 수 있다. 반대로 내부적으로 확률적인 모듈도 입력·출력 계약과 상태가 격리되어 있으면 영향 범위를 좁힐 수 있다.

에이전트에서는 한 단계의 작은 차이가 다음 환경 상태를 바꾼다. 다른 도구를 호출하면 다른 파일과 검색 결과를 보고, 그 결과가 다음 입력이 되어 뒤의 행동 분포도 달라진다. 작업이 길수록 분기할 기회는 늘지만, 실제 폐루프 실행의 단계별 확률은 일정하거나 독립적이지 않으므로 단순한 누적식으로 위험을 정밀하게 계산할 수는 없다. 중요한 점은 영향이 답변 표현에 머물지 않고 에이전트가 방문하는 상태와 긴 작업 궤적에 퍼진다는 것이다.

이 때문에 수정은 사건 수뿐 아니라 **개입의 국소성**으로 나눠야 한다.

| 개입 종류               | 예                                                                                                                                        | 한 사건 뒤의 기본 대응                                                                                                   |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 계약에 국소적인 개입    | 원자적 append·patch, 리비전 충돌 거부, 권한 필터, 하드 타임아웃, 삭제 뒤 비복원 검사                                                      | 고위험 불변식을 즉시 차단하고 결정적 회귀·통합 시험으로 의도한 영향 경계를 확인한다.                                    |
| 비국소적 정책 개입      | 시스템 프롬프트, 시작 자동 주입, 검색 시점, Active Memory 실행 정책, 청킹·상위 K·재순위화·시간 감쇠, Dreaming 승격 기준, 제로 히트 휴리스틱 | 긴급 차단과 분리해 후보로 취급하고, 알려진 작업과 새 궤적을 포함해 반복한 대응 표본 시험이나 무작위 대조 시험에 더해 탐색 평가를 승인 조건으로 둔다. |

첫 번째 종류도 부작용이 없다는 뜻은 아니다. 다만 사후조건과 인터페이스로 영향 범위를 비교적 좁게 만들 수 있다. 두 번째 종류는 A를 고치려고 바꾼 조건이 B·C·D뿐 아니라 이전 정책이 방문하지 않던 E·F 상태를 만들 수 있다. 알려진 회귀 묶음, 여러 모델에서 실행을 반복한 결과, 도구 궤적과 컨텍스트 비교, 탐색적 실패 발견, 외부 효과에 대한 런타임 불변식 감시가 함께 필요한 이유다.

같은 결과 계약을 보호하는 여러 수정안이 있다면 제품 의미, 호환성, 부수 비용이 허용되는 범위에서 인터페이스와 사후조건으로 영향 범위를 더 명확히 묶을 수 있는 안을 우선 검토한다. 국소성만으로 후보를 자동 승인한다는 뜻은 아니며, 각 안이 실제로 같은 계약을 지키는지부터 비교해야 한다.

### 폴백은 성공과 같은 상태가 아니다

QMD가 실패했지만 내장 검색이 두 결과를 반환했다면 주 응답은 계속될 수 있다. 그렇다고 1차 경로가 성공한 실행과 같은 결과는 아니다. 순위, 인용, 신선도, 세션 코퍼스, 지연과 적용된 범위 검사의 순서가 달라질 수 있다. 폴백을 하나의 `success=true`로 접으면 1차 경로의 건강 악화를 숨기고, 대체 경로가 같은 계약을 유지했는지조차 확인하기 어렵다.

대체 경로가 같은 계약을 유지할 수도 있고 일부 의미만 바꾸거나 보장을 약화할 수도 있다. 따라서 폴백은 명시적인 **저하 모드**로 기록해야 한다. 최소한 1차 실패 단계와 원인, 사용한 대체 경로, 폴백 전후 지연과 결과 수, 유지·변경·약화된 계약, 정상 경로로 복귀한 시점을 남긴다. 폴백 발생과 회상 횟수는 진단용 기계 상태가 될 수 있지만, 그 자체를 사용자 의미 메모리의 내용이나 정책 보상의 정답으로 사용해서는 안 된다.

폴백률끼리 비교할 때는 폴백을 사용할 자격이 있었던 전체 실행을 분모로 삼고 질의 난도·라우팅 비율·타임아웃 정책을 가능한 한 맞춘다. 더 어려운 질의가 특정 백엔드로 몰리거나 시간 상한이 엄격해져도 폴백률은 높아질 수 있으므로, 이 비율만으로 백엔드 건강 악화를 단정할 수 없다.

## 메모리 결과 추적부터 시작한다

평가용 말부터 일상어로 맞추자. **합성 평가 실행**은 실제 사적 대화 대신 정답과 허용 범위를 미리 정한 가짜 대화로 시스템을 시험하는 실행이다. **운영 환경 수집**은 실제 사용 중의 사건을 기록하는 일이므로 별도의 명시적 동의가 필요하다. **의미 산출물**은 사람이 나중에도 뜻을 알아볼 수 있는 메모리 기록이고, **수명주기 소유자**는 그 기록·인덱스·삭제 단계를 책임지는 런타임이나 플러그인이다. **평가 사례**(fixture)는 입력과 기대 결과를 묶은 시험 한 건이며, 그 기대 결과를 **정답 표지**(label)라고 부른다.
...
| 인덱스             | `Indexed`             | 내용 리비전 → 검색 가능한 인덱스 리비전                    | 새 청크가 현재 인덱스에 포함됨                      |
| 접근 자격 판정     | `Eligible for access` | 현재 에이전트·대화·질의 → 허용 또는 거부                   | 개인 대화는 허용하고 무관한 그룹 대화는 거부함      |
...
| 주입               | `Injected`            | 검색 후보 → 주 모델이 실제로 받은 컨텍스트                 | `ORBIT-10` 구절이 토큰 예산 안에 들어감             |
| 참조               | `Referenced`          | 모델 컨텍스트 → 답변의 인용·복사·행동 연결                 | 답변이 “현재 코드명은 `ORBIT-10`”이라고 말함        |
| 교정               | `Corrected`           | 사용자 수정·거부 → 파일 편집, 되돌림, 대체 관계            | 사용자가 `ORBIT-11`로 바꾸어 이전 값을 대체함       |
| 삭제               | `Deleted`             | 삭제 요청 → 정본·파생·구체화 사본 처리와 검증 결과         | 재인덱싱·재시작 뒤에도 `ORBIT-10`이 반환되지 않음   |
| 인과 효과          | `Causal lift`         | 반복 대조 실행 → 기억이 만든 개선·피해와 불확실성의 추정치 | 기억을 쓴 조건이 기준 조건보다 정답률을 높였는지 봄 |

이 추적이 있으면 “에이전트가 잊었다”를 하나의 모호한 실패로 취급하지 않아도 된다. 가능한 원인은 다음처럼 나뉜다.

- 기록 단계에서는 쓰기 자체가 없었을 수 있다.
- 인덱스 단계에서는 파생 상태가 원본보다 오래되었을 수 있다.
- 접근 단계에서는 범위 검사에서 거부되었을 수 있다.
- 검색 단계에서는 후보를 놓쳤거나 순위 경쟁에서 밀렸을 수 있다.
- 답변 단계에서는 컨텍스트에서 잘렸거나, 주입된 내용을 답변에서 참조하지 않았을 수 있다.
- 수명주기 단계에서는 내용이 낡았거나 삭제되지 않은 구체화 사본이 남았을 수 있다.

원인마다 확인할 신호와 수정 방법이 다르다.

> **예시 — 합성 사례:** 사용자가 프로젝트 식별자를 `ORBIT-9`에서 `ORBIT-10`으로 정정했다고 하자. `ORBIT-10`이 파일에 써졌더라도 오래된 인덱스가 `ORBIT-9`만 반환하면 `Captured`는 성공하고 `Indexed`에서 실패한 것이다. 둘 다 검색되지만 오래된 값이 앞서면 `Retrieved`의 순위화 문제다. 새 값이 모델 컨텍스트에 들어갔는데 답변이 옛 값을 쓰면 `Injected` 이후의 문제다. 이 예시는 단계 차이를 설명하기 위한 합성 사례다.
...
| 인용된 답변 뒤의 확인·교정         | 그 차례의 관련성과 작업 효용에 관한 증거 | 미래 작업 전반에 걸친 지속적 중요도                        |
| 통제된 QA 또는 대응 표본 섀도 시험 | 명시된 평가 사례와 정책에서의 개선·피해  | 운영 환경 분포 전반의 개선                                 |

`memory_search` → `memory_get` → 인용된 답변, 의미가 다른 질의의 반복, 새 출처가 오래된 충돌을 해결한 경우, 유용·중립·유해 섀도 판정 같은 운영 순서는 유용한 대리 지표다. 단, 무엇의 대리 지표인지 축을 명명해야 한다. 검색 횟수, 순위·점수, 컨텍스트에 머문 시간, 모델의 반복, 단 한 번 성공한 합성 질의만으로는 약한 증거다.

### 고급 실험에서만 선택 편향을 보정한다

현재 Dreaming의 승격 점수에는 회상 빈도가 일부 반영된다. 그러나 자주 검색된 항목은 처음부터 노출 기회가 많았기 때문에 더 유용해 보일 수 있다. 이 효과를 다루려면 고급 실험에서 **자격 모수**(eligible denominator), 즉 검색될 자격이 있었던 전체 횟수와 실제 **노출(exposure)** 횟수, 순위를 함께 기록한다. 무작위 합성 시험이나 대응·교차 섀도 설계가 안전한 곳에서는 각 후보가 선택될 사전 확률인 **선택 성향**(selection propensity)도 기록할 수 있다.
...
| 컴팩션                        | 영속 사실이 사라지거나 플러시가 기존 메모를 덮어씀                |
...
- 직접 대화, 직접 스레드, 비공개 대화 간, 그룹, 공유 스레드 범위
...
결정적인 검색 결과는 확률적으로 달라지는 답변·판정 결과와 분리해 보고한다. 자동 채점기를 쓸 때는 효과 크기, 신뢰 구간, 사람과 채점기의 보정·일치도도 남긴다. 기준선에는 메모리 없음, 최근 컨텍스트만, 현재 기본값, 정답 근거 제공을 포함한다. 전체 컨텍스트 기준선은 같은 컨텍스트·토큰 예산에서 실현 가능하고 공정할 때만 포함한다.

## 비국소적 정책 개입에는 섀도 평가를 승인 조건으로 둔다

Dreaming의 섀도 시험 패턴은 다음처럼 재사용할 수 있다. 특히 프롬프트, 시작 주입, 검색 시점, 청킹, 재순위화, 승격 기준처럼 정책과 이후 궤적을 비국소적으로 바꾸는 변경에서는 이 절차를 선택적인 보조 검토가 아니라 기본 승인 조건으로 삼아야 한다.
...
5. 도구 호출 순서, 읽은 원본, 주입된 컨텍스트와 외부 상태 변경을 비교해 장기 궤적의 분기를 확인한다.
6. 알려진 회귀 사례뿐 아니라 후보 정책이 새로 방문하는 상태를 찾는 탐색 실행을 포함한다.
7. 실패 구간 전반에서 개선이 안정적이고 선언한 안전 경계에 관측된 회귀가 없을 때만 승격하거나 기본값으로 켠다.
...
- 대화 간 본문이나 안정적인 개인 식별자를 기본으로 내보내지 않는다.
- 보존, 삭제, 내보내기 통제를 명시한다.
...
로컬 추적도 사람이 설명할 수 있어야 한다. 예를 들어 “출처 리비전은 인덱싱되었지만 그룹 세션이라 제외됨”, “QMD 시간 초과 뒤 내장 엔진이 인용된 결과 두 개를 반환함”, “결과가 주입되었지만 응답에서 참조되지 않음”처럼 읽혀야 한다. 하나의 불투명한 전역 품질 점수보다 이런 설명 가능성이 더 유용하다.

## 후보 제품 수준 메모리 SLO

서비스 수준 목표(SLO)는 “빠르고 안전해야 한다” 같은 희망이 아니라, **이름을 붙인 작업 부하**를 **정해진 측정 기간** 동안 실행했을 때 지켜야 할 **수치 기준**이다. 허용 가능한 실패량을 **오류 예산**(error budget)이라고 한다. 예를 들어 “대부분 빠르다”는 SLO가 아니지만, “`memory-index-freshness-v1` 합성 작업 부하를 매 출시 후보에서 1,000회 실행해 99%가 5초 안에 검색 가능해야 하며, 5초를 넘는 실행은 최대 10회”라고 쓰면 측정 가능한 약속이 된다.

다음 두 항목은 형식을 보여 주기 위한 **가상 수치 예시**이며 현재 제품 보장이나 권고된 최종 임계값이 아니다.

- **범위 안전 SLO 예시:** `memory-scope-conformance-v1`의 직접 대화·비공개 스레드·그룹·공유 스레드·교차 에이전트 조합을 매 출시 후보에서 모두 실행한다. 허용되지 않은 인용·주입·Dreaming 추적은 0건이어야 하며 오류 예산도 0건이다. 한 건이라도 나오면 출시 관문을 통과하지 못한다.
- **인덱스 신선도 SLO 예시:** `memory-index-freshness-v1`의 합성 Markdown 쓰기 1,000건을 매 출시 후보에서 실행한다. 99%는 5초 안에 같은 내용 리비전으로 검색 가능해야 하고, 5초 초과 오류 예산은 10건이다. 공급자 장애 시험은 이 작업 부하와 섞지 않고 별도 폴백 SLO로 측정한다.
...
결과 추적은 “기억이 나빴다”는 인상을 쓰기 부재, 인덱스 지연, 범위 거부, 검색 실패, 주입 손실, 낡은 내용처럼 서로 다른 원인으로 나눈다. 이 분해를 실제 평가에 쓰려면 먼저 현재 소스·테스트·문서가 서로 같은 계약을 말하는지 확인해야 한다.
