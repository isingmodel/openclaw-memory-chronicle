...
2026-07-18 기준 `main`에서 가장 즉각적인 세 간극은 문서와 런타임의 비공개 부트스트랩 불일치, Dreaming 저장소 문서의 노후화, 대화 기록 물질화의 별도 범위 경계다. 이 장은 여기에 의미 설명, 삭제, 백업·복구·내보내기, 재설정 포획, 플러그인 적합성과 도구 라우팅의 계약 공백도 함께 기록한다. 이 판단은 오래된 이슈 설명을 옮긴 것이 아니라 고정본의 소스, 테스트, 문서를 대조한 결과다.
...
따라서 이 항목은 더 이상 닫아야 할 현재 계약 간극이 아니다. 다만 개발 선두 문서를 이전 릴리스 실행 파일과 함께 읽으면 잠시 존재하지 않는 기본값을 볼 수 있다는 일반적인 판본 위험은 남는다. 호환성과 프라이버시에 민감한 설정 문서에는 최초 지원 버전 또는 눈에 띄는 출시 범위가 필요하다. **마지막 출시 상태 검증: 2026-07-26.**

<a id="deletion-contract-gap"></a>

## 확인된 간극 · 제안 우선순위 P1: 삭제 결과가 저장소 전반에 정의되어 있지 않다

[1장의 삭제 책임표](01-current-architecture.md#삭제는-검색의-반대말이-아니다)는 같은 사실이 Markdown, 파생 색인, 정식 대화 기록, Dreaming 증거, 가져온 파일과 대안 플러그인에 서로 다른 형태로 남을 수 있음을 보여 준다. 현재 각 소유자는 자기 저장소의 삭제나 재구축을 일부 지원하지만, “이 사실을 잊어라”를 모든 관련 복사본에 적용하고 완료를 증명하는 하나의 메모리 삭제 계약은 없다.
...
2. **권위 원본 처리:** Markdown, 대화 기록, LanceDB 같은 각 정본 소유자가 삭제 또는 교정을 수행한다. 파생 색인만 지워 원본에서 다시 살아나는 상태를 성공으로 보지 않는다.
3. **파생·물질화 정리:** FTS·벡터·QMD, `source=sessions`, Dreaming 증거와 가져오기 사본을 다시 동기화하거나 명시적으로 삭제한다.
4. **검증:** 권한이 허용된 각 검색 경로에서 삭제 대상이 반환되지 않고, 재시작·재색인 뒤에도 되살아나지 않는지 확인한다.
5. **보존 예외:** 감사·법적 보존·백업 때문에 즉시 지울 수 없는 사본이 있으면 위치, 보존 기간, 접근 범위와 최종 삭제 시점을 사용자에게 알린다.

이 책의 제안은 삭제를 “검색 결과 없음”으로만 측정하지 않는 것이다. 원본 제거, 파생 상태 수렴, 물질화 사본 처리, 재생성 방지를 서로 다른 사건으로 추적해야 한다. 삭제 실패는 정확도 결함이 아니라 보존·프라이버시 계약 위반일 수 있다.
...
따라서 “워크스페이스를 복사했다”는 말만으로 완전한 복구를 보장할 수 없다. 반대로 모든 캐시와 색인을 보존해야 하는 것도 아니다. 백업 계약은 다음 세 부류를 나눠야 한다.

- **반드시 보존할 정본:** 사용자가 편집하는 Markdown, 정식 대화 기록, 재구축할 수 없는 운영·감사 상태, 플러그인이 소유한 의미 원본
- **선택적으로 다시 만들 파생 상태:** 내장 FTS·벡터 색인, QMD 색인, Memory Wiki 컴파일 캐시
- **이름 있는 이동 산출물:** 사용자가 요청한 내보내기, 가져오기 원본, 제한된 이벤트 내보내기

가져오기는 7월에 제품 경로가 되었지만, 반대 방향의 **메모리 전체 내보내기**와 다른 하네스에서의 복원 검증은 같은 수준으로 정의되지 않았다. 몇 년 동안 커진 `MEMORY.md`, 일일 기록, 대화 기록과 색인의 보존 기간·비용·정리 목표도 하나의 운영 예산으로 묶여 있지 않다.

이 책은 백업 명세에 소유자, 포함·제외 목록, 암호화, 일관된 스냅샷 시점, 복원 순서와 재색인 단계를 넣고, 복원 시험을 정기 평가에 포함할 것을 제안한다. 내보내기는 사람이 읽는 정본과 기계 상태를 구분하고, 장기 성장 평가는 저장량뿐 아니라 색인 시간, 첫 검색 지연, 재구축 비용, 삭제 수렴 시간을 함께 측정해야 한다.
...
현재 계약의 간극은 그대로 평가 의무가 된다. 비공개 부트스트랩, 대화 기록 물질화, 저장소 설명, 오염 방지, 삭제, 백업·복구, 플러그인 소유권이 합의되지 않으면 높은 회상 점수도 범위와 수명주기를 지켰다는 증명이 될 수 없다.
