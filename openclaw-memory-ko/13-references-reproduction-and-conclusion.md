...
이 명령 묶음은 한 파일의 계보, 특정 시점의 내용, 기능 소유권이 이동한 경로, 커밋이 현재 시점에 포함되는지 여부, 커밋 포함 관계로 찾은 가장 이른 태그, 외부에 기록된 동기를 각각 교차 확인하게 해 준다. 태그가 존재한다는 사실만으로 기능이 그 태그에서 실제 설정·마이그레이션까지 온전히 동작했다고 추론해서는 안 된다.
...
- 최초 포함 태그 대응은 아키텍처 고정본과 분리해 2026-07-26에 다시 검증했다. 커밋 포함 관계와 태그 생성 순서를 사용하되 그 검증일 뒤에 생성된 태그는 제외했다. 고정본 뒤에 생긴 태그는 최초 출시 상태만 갱신하며 본문의 2026-07-18 현재 계약에는 섞지 않았다. 이는 출시 경계를 찾는 보조 수단이지 설치 현황 조사가 아니다.
...
- 8장부터 11장의 결과 계약·능력 스캐폴딩 분류와 가드 수명주기는 앞선 코드 사실에서 도출한 이 책의 분석·관리 제안이지, 유지보수자가 선언한 현재 계약이나 측정된 우열이 아니다. 스캐폴딩 제거 평가도 실행하지 않았으므로 특정 가드가 이미 불필요하다거나 얇은 하네스가 더 낫다고 판정하지 않는다.
