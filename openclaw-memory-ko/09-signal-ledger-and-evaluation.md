...
| [이슈 #45608](https://github.com/openclaw/openclaw/issues/45608): 초기화 전 에이전트식 플러시 부재                                   | 기계적 대화 기록 포획과 의미 통합은 같지 않다                           | 지금도 열려 있음                                                                                            | `/new`에 제한된 큐레이션을 넣어야 하는가? 지연과 데이터 손실의 균형은 어떻게 잡는가?                                  |
...
| [이슈 #99611](https://github.com/openclaw/openclaw/issues/99611): 표면을 넘나들 때의 기억상실                                        | 개인 비서 식별 정보에 비해 세션 키가 지나치게 좁았다                    | 보호된 `rememberAcrossConversations`. 현재의 `main`에만 있고 출시 태그에는 없음                             | 열거한 모든 바인딩·DM 배치 적합성 사례에서 관찰된 교차 사용자 회상이 0이고, 식별 불가 대상은 계속 실패 시 차단되는가? |
